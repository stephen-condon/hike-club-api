...
- [ ] **API-VER-010**: The system shall evaluate a version's sunset against a current time supplied to it, so that sunset enforcement is exercised in tests without the Workers runtime clock.
